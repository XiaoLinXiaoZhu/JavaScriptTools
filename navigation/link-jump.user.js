...
// @name                中间页自动跳转
// @name:zh-CN          中间页自动跳转
// @name:en             Auto Redirect — Skip intermediate link pages
// @namespace           https://github.com/XiaoLinXiaoZhu/JavaScriptTools
// @version             0.1.0
// @description         自动跳过 CSDN、知乎、豆瓣、微博、简书、Gitee、掘金等 30+ 网站的中间确认页，直接跳转到目标链接。
// @description:en      Automatically skip intermediate confirmation pages on 30+ Chinese websites (CSDN, Zhihu, Weibo, Juejin, etc.) and redirect directly to the target URL.
// @author              XLXZ
// @license             MIT
...
// @match               *://weboffice.qq.com/scenario/link.html*
// @grant               GM_getValue
// @grant               GM_setValue
// @grant               GM_registerMenuCommand
// @downloadURL         https://xiaolinxiaozhu.github.io/JavaScriptTools/navigation/link-jump.user.js
// @updateURL           https://xiaolinxiaozhu.github.io/JavaScriptTools/navigation/link-jump.user.js
// ==/UserScript==
...
` + cl, document.head.appendChild(e), Ms = true;
  ...
  var STORAGE_PREFIX = "linkJump_";
  function isEnabled(name) {
    const key = STORAGE_PREFIX + name;
    const stored = GM_getValue(key, void 0);
    return stored === void 0 ? true : stored;
  }
  ...
  function extractUrl(rule) {
    ...
    if (rule.selector) {
      const el2 = document.querySelector(rule.selector);
      if (el2?.textContent) {
  ...
  function jump(url) {
    ...
    hl("\u6B63\u5728\u8DF3\u8F6C...", { type: "success", duration: 1500 });
    setTimeout(() => {
  ...
  __name(findMatchingRule, "findMatchingRule");
  function run() {
    const rule = findMatchingRule();
    if (!rule) return;
    if (!isEnabled(rule.name)) {
      hl(`\u5DF2\u5173\u95ED\u300C${rule.name}\u300D\u7684\u81EA\u52A8\u8DF3\u8F6C`, { type: "warning", duration: 3e3 });
      return;
    }
    ...
    if (targetUrl) {
      ...
      hl("\u672A\u80FD\u89E3\u6790\u5230\u76EE\u6807\u94FE\u63A5\uFF0C\u8BF7\u624B\u52A8\u8DF3\u8F6C", { type: "warning", duration: 3e3 });
    }
  }
  __name(run, "run");
