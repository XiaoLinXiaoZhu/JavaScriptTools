...
(() => {
  ...
  // src/obsidian-cardview/index.ts
  var css = `
<style>
...
`;
  var _CardSystem = class _CardSystem {
    constructor() {
      this.state = {
        categoryFilter: null,
        tagFilters: /* @__PURE__ */ new Set(),
        columns: 3,
    ...
    renderMarkdown(content) {
      return content.replace(
        /^#(#+) (.*)$/gm,
        (_, level, text) => `<h${level.length + 1}>${text}</h${level.length + 1}>`
      ).replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>").replace(/\*(.*?)\*/g, "<em>$1</em>").replace(
        /!\[\[(.*?)\]\]/g,
        (_, img) => this.resolveImage(img)
      ).replace(
        /\[\[(.*?)\]\]/g,
    ...
    resolveImage(filename) {
      ...
      return imgPath ? `<img src="${imgPath}" alt="${filename}" class="obs-image" onerror="this.classList.add('image-error')">` : `<div class="image-error">\u56FE\u7247\u672A\u627E\u5230: ${filename}</div>`;
    }
    ...
    applyFilters() {
      this.cards.forEach((card) => {
        const tagMatch = this.state.tagFilters.size === 0 || card.tags.some((t) => this.state.tagFilters.has(t));
        const categoryMatch = !this.state.categoryFilter || card.category === this.state.categoryFilter;
      ...
      <div class="toolbar">
        <div class="filter-group">
          ...
          <select class="category-select"
            ...
            ${categories.map(
        (c) => `<option value="${c}" ${this.state.categoryFilter === c ? "selected" : ""}>${c}</option>`
      ).join("")}
          </select>
        </div>
    ...
    toggleTag(tag) {
      this.state.tagFilters.has(tag) ? this.state.tagFilters.delete(tag) : this.state.tagFilters.add(tag);
      this.update();
    }
    resetFilters() {
      this.state = {
  ...
  __name(_CardSystem, "CardSystem");
  var CardSystem = _CardSystem;
